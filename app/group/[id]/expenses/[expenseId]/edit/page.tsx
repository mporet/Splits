...
export default function EditExpensePage() {
    ...
    useEffect(() => {
        if (!expenseId || participants.length === 0) return;

        fetch(`/api/groups/${id}/expenses/${expenseId}`)
            .then(res => {
            ...
            .then(expense => {
                ...
                setAmountInput((expense.amount / 100).toString());
                setCurrency(expense.currency);

                // Setup Payers
                const isPayerExact = expense.payers.some((p: any) => {
                    // It's considered exact if any amount isn't exactly the equal share (accounting for remainder)
                    // Or more simply, let's just infer from the data structure, but since we don't store exactly if it was EQUAL vs EXACT for payers...
                    // Let's assume EXACT if there's more than 1 payer and they aren't precisely equal
                    return false; // Actually, let's just default to EXACT for editing payers if there's multiple to be safe, or just reconstruct.
                });

                // Let's deduce payer mode based on if all amounts are roughly equal
                const payerMap: Record<string, boolean> = {};
                const exactPaymentMap: Record<string, string> = {};
                let allEqualPayer = true;
                const payerShare = Math.floor(expense.amount / expense.payers.length);

                expense.payers.forEach((p: any) => {
                    payerMap[p.participantId] = true;
                    exactPaymentMap[p.participantId] = (p.amountPaid / 100).toString();
                    if (Math.abs(p.amountPaid - payerShare) > 1) {
                        allEqualPayer = false;
                    }
                });

                setSelectedPayers(payerMap);
                setExactPayments(exactPaymentMap);
                setPayerMode(allEqualPayer ? "EQUAL" : "EXACT");

                // Setup Splits
                const splitMap: Record<string, boolean> = {};
                const exactSplitMap: Record<string, string> = {};
                const percentageSplitMap: Record<string, string> = {};
                let commonSplitType = "EQUAL";

                if (expense.splits.length > 0) {
                    commonSplitType = expense.splits[0].splitType; // Assuming all splits in an expense have the same type
                }

                expense.splits.forEach((s: any) => {
                    splitMap[s.participantId] = true;
                    if (commonSplitType === "EXACT") {
                        exactSplitMap[s.participantId] = (s.amountSplit / 100).toString();
                    } else if (commonSplitType === "PERCENTAGE") {
                        percentageSplitMap[s.participantId] = (s.percentage || 0).toString();
                    }
                });

                setSelectedSplits(splitMap);
                setExactSplits(exactSplitMap);
                setPercentageSplits(percentageSplitMap);
                setSplitMode(commonSplitType as any);
    ...
    const toggleSelection = (
        ...
        setState(prev => ({ ...prev, [pid]: !prev[pid] }));
    };
    ...
    if (fetchingExpense) {
        return <div className="container mt-4 text-center">Loading expense details...</div>;
    }
        ...
        <div className="container">
            <div className="header mb-4">
                <div>
                    ...
                    <p className="subtitle">Update the details of the purchase</p>
                </div>
